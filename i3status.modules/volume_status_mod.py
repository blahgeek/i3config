...
class Py3status:
    ...
    def _prec_to_format(self, string):
        try:
            value = int(string)
        except ValueError:
            return self.format_muted

        if value < self.threshold_degraded:
            return self.format_degraded
        else:
            return self.format
    ...
    def current_volume(self):
        ...
        text = self._format_output(self.format_muted
                                   if muted else self._prec_to_format(perc if not muted else '0'), perc)
